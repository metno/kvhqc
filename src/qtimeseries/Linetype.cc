...
{
  const std::string ltname = miutil::to_lower(_name);
  std::map<std::string,Linetype>::iterator p;
...
// static
void Linetype::define(const std::string& _name, uint16 _bmap, int _factor)
{
  const std::string ltname = miutil::to_lower(_name);
  Linetype lt;
  lt.name=    ltname;
