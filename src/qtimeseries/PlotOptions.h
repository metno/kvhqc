...
#ifndef _PlotOptions_h
#define _PlotOptions_h
...
namespace POptions {
  ...
  struct PlotOptions {
    ...
    PlotOptions():
      drawgrid(true), gridTimes(1), language("EN"), plottype(type_line),
      textcolour(BLACK), linecolour(BLACK), fillcolour(WHITE),
      bordercolour(BLACK),timecolour(BLACK),
      linewidth(1), axis(axis_left_left),
      axisFixedMin(false), axisMin(0), axisFixedMax(false), axisMax(100),
      marker(NO_MARKER)
      //polystyle(poly_fill),
