...
void EditAccess::addEditTimes(TimeSet& times, const std::vector<Sensor>& sensors, const TimeRange& limits)
{
...
ObsAccess::TimeSet EditAccess::allTimes(const std::vector<Sensor>& sensors, const TimeRange& limits)
{
    TimeSet times = mBackend->allTimes(sensors, limits);
    addEditTimes(times, sensors, limits);
    return times;
}

ObsAccess::DataSet EditAccess::allData(const std::vector<Sensor>& sensors, const TimeRange& limits)
{
  METLIBS_LOG_SCOPE();
  DataSet data;
  BOOST_FOREACH(const Sensor& sensor, sensors) {
  ...
  const DataSet dataBackend = mBackend->allData(sensors, limits);
